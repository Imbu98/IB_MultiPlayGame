...
void AIB_RPGPlayerController::SwitchController()
{
	if (!HasAuthority())
	{
		...
		if (AIB_MainChar* IB_MainChar = Cast<AIB_MainChar>(GetPawn()))
		{
			...
			if (OwningCannon)
			{
				...
				Possess(OwningCannon);
					OwningCannon->ClientSetCannonInfo(this,IB_MainChar);
		...
		//  Cannonn에서 IB_MainChar Possess
		else if (ACannonPawn* Cannon = Cast<ACannonPawn>(GetPawn()))
		{
			IsOnCannon = false;
			Possess(CachedIB_MainChar);
			OwningCannon->DefaultSceneRoot->SetVisibility(false);
			OwningCannon->ClientSetCannonInfo(nullptr,nullptr);
			FTimerHandle TimerHandle;
			GetWorld()->GetTimerManager().SetTimer(TimerHandle, [this]()
				{
					ClientSwitchWidget();
